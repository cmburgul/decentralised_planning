...
int main(int argc, char** argv)
{
	...
	if (argc != 5)
    {
        ROS_INFO_STREAM("usage: rosrun get_plan_client [agent_<id_number>] [goal_x] [goal_y] [goal_yaw]");
    }
	
	else
	{
		ros::NodeHandle nh;
		ros::ServiceClient client = nh.serviceClient<decentralised_planning::GetPath>("get_plan");
        ...
        if (client.call(srv))
        {
            ...
            ROS_INFO("Visualize the Path in Rviz");
